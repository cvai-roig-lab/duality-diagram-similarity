...
def main():
    ...
    num_images_range = range(10,500,10) # for 2 random indices
    num_repetitions = 100
    ...
    kernel_type = ['rbf','lap','linear']#,'rbf','lap']
    similarity_type = ['cka'] #'spearman',
    ablation_type = ['debiased_centered'] #'default',
    feature_ablation_type = ['znorm']#,'group_norm','instance_norm','layer_norm','batch_norm']#'z_norm_my_implementation']#'covLF',
    save_dir = './CVPR2020_results/pascal_normcomparison_timing_' + dataset
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    for num_images in num_images_range:
        if True:
            save_path = os.path.join(save_dir,str(num_images).zfill(3) + '_kernels.npy')
            affinity_ablation = {}
        ...
        save_path = os.path.join(save_dir,str(num_images).zfill(3) + '_rdms.npy')
        dist_type = ['pearson', 'euclidean', 'cosine']
        affinity_ablation = {}
