...
def main():
    ...
    for model_number in model_numbers:
        pascal_feats_path = '/home/kshitij/projects/taskonomy/deeplab_v3/CVPR2020_small_feat_pascal_pascal_200/' + str(model_number) + '/'
        ...
        kernel_type = ['lap']#,'rbf','lap']
        similarity_type = ['spearman','cka'] #'spearman',
        ablation_type = ['default','debiased_centered'] #'default',
        feature_ablation_type = ['centering','znorm']#,'group_norm','instance_norm','layer_norm','batch_norm']#'z_norm_my_implementation']#'covLF',
        save_dir = './CVPR2020_results/pascal_normcomparison_iters_' + dataset
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        if True:
            save_path = os.path.join(save_dir, str(model_number).zfill(6) + '_kernels.npy')
            affinity_ablation = {}
                    ...
                    for ablation in (ablation_type):
                        ...
                        for feature_ablation in feature_ablation_type:
                            ...
                            for i in range(num_repetitions):
                                indices.append(np.arange(num_images))
        ...
        save_path = os.path.join(save_dir,str(model_number).zfill(6) +'_rdms.npy')
        dist_type = ['pearson']
        affinity_ablation = {}
